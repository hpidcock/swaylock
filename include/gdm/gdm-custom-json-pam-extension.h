...
#define GDM_PAM_EXTENSION_CUSTOM_JSON \
        "org.gnome.DisplayManager.UserVerifier.CustomJSON"

#define GDM_PAM_EXTENSION_CUSTOM_JSON_SIZE \
...
{ \
        size_t proto_len = strnlen((proto_name), \
                sizeof((request)->protocol_name) - 1); \
        GDM_PAM_EXTENSION_LOOK_UP_TYPE(GDM_PAM_EXTENSION_CUSTOM_JSON, \
                &((request)->header.type)); \
        (request)->header.length = \
                htobe32(GDM_PAM_EXTENSION_CUSTOM_JSON_SIZE); \
...
{ \
        size_t proto_len = strnlen((proto_name), \
                sizeof((response)->protocol_name) - 1); \
        GDM_PAM_EXTENSION_LOOK_UP_TYPE(GDM_PAM_EXTENSION_CUSTOM_JSON, \
