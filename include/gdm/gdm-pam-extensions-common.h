/*
 * Based on https://gitlab.gnome.org/GNOME/gdm/-/blob/8c3fbd5ddefb8097d990a12581453582ec71a32f/pam-extensions/gdm-pam-extensions-common.h
 *
...
#define GDM_PAM_EXTENSION_MESSAGE_FROM_PAM_MESSAGE(query) \
        ((GdmPamExtensionMessage *)(void *)(query)->msg)

#define GDM_PAM_EXTENSION_MESSAGE_TO_PAM_REPLY(msg) \
        ((char *)(void *)(msg))
...
#define GDM_PAM_EXTENSION_MESSAGE_TRUNCATED(msg) \
        (be32toh((msg)->length) < sizeof(GdmPamExtensionMessage))
...
#define GDM_PAM_EXTENSION_MESSAGE_MATCH(msg, supported_extensions, name) \
        (strcmp((supported_extensions)[(msg)->type], (name)) == 0)

/*
 * environment_block should be a statically-allocated chunk of memory.
 * This is important because putenv() will leak otherwise (and setenv
 * isn't thread safe).
 */
...
{ \
        ...
        _p = stpncpy(_p, "GDM_SUPPORTED_PAM_EXTENSIONS", \
                sizeof(environment_block)); \
        *_p = '\0'; \
        _size += strlen(environment_block); \
        for (_t = 0; \
             (supported_extensions)[_t] != NULL && _t < UCHAR_MAX; \
             _t++) { \
                size_t _next_chunk = \
                        strlen((supported_extensions)[_t]) + strlen(" "); \
...
#define GDM_PAM_EXTENSION_SUPPORTED(name) \
        GDM_PAM_EXTENSION_LOOK_UP_TYPE((name), (unsigned char *)NULL)

#endif /* GDM_PAM_EXTENSIONS_COMMON_H */